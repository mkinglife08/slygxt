...
// 配置
layui.config({
    base: __curHostAddress + 'Scripts/'  // 模块目录
}).extend({                     // 模块别名
    vip_nav: 'vip_nav'
    , vip_tab: 'vip_tab'
    , vip_table: 'vip_table'
...
layui.use(['layer', 'element', 'util'], function () {
    ...
    function navHide(t, st) {
        var time = t ? t : 50;
        st ? localStorage.log2 = 1 : localStorage.log2 = 0;
    ...
    function navShow(t, st) {
        var time = t ? t : 50;
        st ? localStorage.log2 = 0 : localStorage.log2 = 1;
        ...
        if (flag > 0) {
            ...
            if (src) {
                ...
                element.tabAdd(card, {
                    ...
                    , content: '<iframe src="' + src + '" frameborder="0" id="tab_iframe_' + id + '"></iframe>'
                    , id: id
                });
                // 关闭弹窗
                layer.closeAll();
    ...
    function init() {
        ...
        // 设置高度
        $(document).find(".layui-tab[lay-filter='card'] div.layui-tab-content").height($(window).height());
    }
