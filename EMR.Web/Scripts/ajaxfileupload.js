...
function ajax_upload(obj, type,curid, callback) {
    //具体的上传图片方法，obj：上传控件，type：图片类型（这里的类型指的是用户图片、模块图标、功能图标等图片对应的服务名称，服务名称用于通过接口获取主键ID，该名称区分大小写！！！）
    if (image_check(obj.id)) { //自己添加的文件后缀名的验证
        $.ajaxFileUpload({
            fileElementId: obj.id,    //需要上传的文件域的ID，即<input type="file">的ID。
            url: __curHostAddress + 'api/SystemSupport/User/SaveFiles?token=' + EWinsBase.Token + '&type=' + type + '&curid=' + curid, //后台方法的路径
